...
usage() {
  cat <<'EOF'
Usage: scripts/re-review-comment.sh <pr-number> [--dry-run] [--edit-last]

Generate a re-review comment with Codex after author follow-up and either print it
...
issue_comments=$(
  gh api "repos/$repo_slug/issues/$pr_number/comments" --paginate \
    --jq '.[] | "- [" + (.user.login // "unknown") + "] " + (.created_at // "") + "\n" + (.body | gsub("\r"; ""))'
)
review_comments=$(
  gh api "repos/$repo_slug/pulls/$pr_number/comments" --paginate \
    --jq '.[] | "- " + (.path // "unknown") + ":" + ((.line // 0) | tostring) + " [" + (.user.login // "unknown") + "] " + (.body | gsub("\r"; ""))'
)
...
cat >"$prompt_file" <<__CODEX_REREVIEW_PROMPT__
You are re-reviewing GitHub pull request #$pr_number for $repo_slug.
...
- Determine whether previously raised P0/P1 findings are actually resolved.
...
Before you finalize:
- Use the exact re-review format from refs/AGENTS.md if one is defined.
- If the author's response does not clearly resolve a prior P0/P1, keep it open.
- Prefer concrete file:line references when possible.
- If all P0/P1 items are resolved, explicitly approve merge.
__CODEX_REREVIEW_PROMPT__

codex exec -s read-only -o "$output_file" - <"$prompt_file"
[[ -s "$output_file" ]] || die "Codex returned an empty re-review"
